...
- セッションは UDP を使うため、TCP しか通らない環境 (一部の制限付きネットワーク) では確立できない。その場合は [Tailscale 経由の Eternal Terminal 接続](tailscale-eternal-terminal.md) を使う。Eternal Terminal は TCP で接続するため、UDP がブロックされた環境でもセッションを維持できる
